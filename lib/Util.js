...
Util.saveNewMainData = function (mainData) {
    ...
    mainData.OpportunityCode = Util.getOpportunityCode();
    return new Promise(function (resolve, reject) {
        let connection = new Connection(tediousConfig);
        connection.on('connect', function (err) {
            if (err) {
            ...
            // purify data
            mainData.ReviewerID = Number(mainData.ReviewerID);
            ...
            let sqlStr = 'insert into MainData (ReviewerID, BUDistrictID, OpportunityCode, Province, City, Site, ChineseName, ' +
                ...
                1 + ',' +
                'N\'' + 1 + '\'' +
                // Number(mainData.ReviewerID) + ',' +
                // Number(mainData.BUDistrictID) + ',' +
                // // to do compute the opprotunity code
                ...
                // Number(mainData.PipelineStatusID) + ',' +
                // Number(mainData.ContractTermID) + ',' +
                // Number(mainData.TargetRateID) + ',' +
                // Number(mainData.AnnualSales) + ',' +
                // 'N\'' + mainData.CorporateAccountChinese + '\',' +
                // 'N\'' + mainData.CorporateAccountEnglish + '\',' +
                // 'N\'' + mainData.SalesRep + '\',' +
                // Number(mainData.AssistCAMNameID) + ',' +
                // Number(mainData.FollowingStatusID) + ',' +
                // Number(mainData.CTCBUID) + ',' +
                // 'N\'' + mainData.CTCSales + '\',' +
                // Number(mainData.SalesTypeID) + ',' +
                // 'N\'' + mainData.FollowingStatusRemark + '\',' +
                // Number(mainData.CompetitorCNID) + ',' +
                // 'N\'' + mainData.FirstCollaborationDate + '\',' +
                // Number(mainData.EstimatedPCO) + ',' +
                // 'N\'' + mainData.Remark + '\',' +
                // Number(mainData.MarketClassificationID) + ',' +
                // 'N\'' + mainData.Username + '\'' +
                ') SELECT @@IDENTITY';
            let request = new Request(sqlStr, function (err) {
                if (err) {
