...
function generateReviewSql(username) {
    let reviewFilePath = 'manage-config.json';
...
Util.checkReviewRight = function(managerName, staffName) {
    ...
    for (var index in reviewConfig) {
        if (String(reviewConfig[index].manager).toLowerCase() === String(managerName).toLowerCase()) {
            for (var j in reviewConfig[index].staff) {
                if (String(reviewConfig[index].staff[j]).toLowerCase() === String(staffName).toLowerCase()) {
                    return true;
                }
            }
        }
...
Util.getManagerConfigs = function () {
    let manageFilePath = 'manage-config.json';
    let managerConfigs = JSON.parse(fs.readFileSync(manageFilePath));
    return managerConfigs;
}
Util.saveManagerConfigs = function (managerConfigs) {
    let manageFilePath = 'manage-config.json';
    fs.writeFileSync(manageFilePath, JSON.stringify(managerConfigs));
}
Util.getSingleManagerConfig = function (username) {
    let manageConfig = Util.getManagerConfigs();
    for (var index in manageConfig) {
        if (String(manageConfig[index].manager).toLowerCase() === String(username).toLowerCase()) {
            return manageConfig[index];
        }
    }
    return null;
}
Util.checkManagerIdentity = function (username) {
    let manageConfig = Util.getManagerConfigs();
    for (var index in manageConfig) {
        if (String(manageConfig[index].manager).toLowerCase() === String(username).toLowerCase()) {
            return true;
